 ...
 // ─── SMS helper (Green Web / any BD SMS gateway) ───────────────────────────
 const sendSMS = async (phone, message) => {
   try {
     const apiKey  = process.env.SMS_API_KEY;
     const sender  = process.env.SMS_SENDER_ID || "YourApp";
     if (!apiKey) return; // SMS disabled if no key
 ...
 // ─── ADMIN: all requests (with phone, name, balance) ──────────────────────
 exports.getAllWithdraw = async (req, res) => {
   try {
     ...
     const data = await Withdraw.find(filter)
       .populate("user", "phone name balance")   // ← phone added
       .sort({ createdAt: -1 });
