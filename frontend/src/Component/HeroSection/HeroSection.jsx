...
const Hero = () => {
  ...
  return (
    <>
      {/* Install Modal */}
      {showInstallModal && (
        <div className="fixed inset-0 z-[9999] bg-black/70 flex items-end justify-center px-4 pb-6">
          <div className="bg-white rounded-3xl w-full max-w-sm p-6 shadow-2xl">
            <div className="text-center mb-5">
              <div className="text-4xl mb-2">📱</div>
              <h2 className="text-xl font-black text-slate-800">অ্যাপ ইন্সটল করুন</h2>
              <p className="text-sm text-slate-500 mt-1">নিচের ধাপগুলো অনুসরণ করুন</p>
            </div>

            <div className="space-y-3 mb-6">
              <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4">
                <p className="font-bold text-sm text-orange-800 mb-1">🌐 Chrome / Samsung Browser</p>
                <p className="text-xs text-orange-700">উপরের ⋮ মেনু → <b>"Add to Home Screen"</b> → Add</p>
              </div>
              <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4">
                <p className="font-bold text-sm text-blue-800 mb-1">🍎 iPhone / Safari</p>
                <p className="text-xs text-blue-700">নিচের Share বাটন → <b>"Add to Home Screen"</b> → Add</p>
              </div>
            </div>

            <button
              onClick={() => setShowInstallModal(false)}
              className="w-full bg-orange-500 text-white py-3 rounded-2xl font-bold active:scale-95 transition"
            >
              বুঝেছি ✓
            </button>
          </div>
          ...
          <div className="w-full md:w-1/2 text-center md:text-left order-2 md:order-1">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 mt-6">
              {[
                ...
                <div key={index} className="animate-custom-float" style={{ animationDelay: `${index * 0.4}s` }}>
                  <span className="cursor-default bg-white/10 backdrop-blur-md border border-white/20 text-white text-xs md:text-sm px-4 py-2 rounded-full flex items-center gap-2 shadow-lg">
                    <span className={`w-2.5 h-2.5 ${game.color} rounded-full animate-pulse`} style={{ boxShadow: `0 0 10px ${game.glow}` }}></span>
            ...
            <div className="mt-10 flex flex-col sm:flex-row gap-5 justify-center md:justify-start">
              <button
                ...
                className="bg-[#ff8a00] hover:bg-orange-600 px-10 py-4 rounded-2xl font-black text-white shadow-[0_10px_30px_rgba(0,0,0,0.3)] transition-all transform active:scale-95"
              >
                📱 এখনই ডাউনলোড করুন
              </button>
              ...
              <div className="relative w-[280px] md:w-[330px] aspect-[9/18.5] bg-[#020617] rounded-[3rem] border-[10px] border-[#1e293b] shadow-2xl overflow-hidden ring-4 ring-white/5">
                ...
                <div className="h-full w-full bg-slate-50 flex flex-col items-center justify-center p-8 text-center">
                  <div className="w-24 h-24 bg-gradient-to-tr from-[#ff3d00] to-[#ff8a00] rounded-[2rem] flex items-center justify-center shadow-2xl mb-6">
                    <span className="text-white font-black text-base leading-tight uppercase tracking-tighter">uthi<br/>YO</span>
                  </div>
