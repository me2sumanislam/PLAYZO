  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundImage:
            "linear-gradient(#ff7a1a 1px, transparent 1px), linear-gradient(90deg, #ff7a1a 1px, transparent 1px)",
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-14 relative z-10">
        ...
        <div className="w-full md:w-1/2 text-center md:text-left order-2 md:order-1">
          <span className="font-arena inline-flex items-center gap-2 text-[#ff7a1a] text-xs md:text-sm font-bold tracking-[0.3em] uppercase border border-[#ff7a1a]/30 px-3 py-1 rounded-sm">
            <span className="w-1.5 h-1.5 bg-[#ff7a1a] rounded-full live-dot" />
          ...
          <h1 className="mt-5 text-4xl md:text-6xl font-black text-white leading-[1.05]">
            বাংলাদেশের সেরা
            <br />
            <span className="text-[#ff7a1a]">টুর্নামেন্ট</span> অ্যাপ
          </h1>
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mt-7">
            {games.map((game, index) => (
              <span
                key={index}
                className="font-arena flex items-center gap-2 bg-white/[0.08] border-l-2 border-[#ff7a1a] pl-3 pr-4 py-2 text-white text-sm font-bold tracking-wide"
              >
                <span className="text-[#ff7a1a] text-[10px] font-black">
                  {game.code}
                </span>
                {game.name}
              </span>
            ))}
            <span className="text-white/60 text-sm italic px-2">
              + আরও অনেক গেম
            </span>
          </div>

          <p className="mt-8 text-white/80 font-medium text-base md:text-lg max-w-xl mx-auto md:mx-0 leading-relaxed">
          ...
          <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
            <button
              ...
              className="relative bg-[#ff7a1a] hover:bg-[#f0700f] active:bg-[#e0660a]
                         text-white font-arena font-bold py-4 px-10 text-lg
            ...
            <button className="border border-white/30 text-white font-arena font-bold px-8 py-4 hover:border-white hover:bg-white/10 transition-all active:scale-95">
              ভিডিও দেখুন
            </button>
          </div>
        ...
        <div className="w-full md:w-1/2 flex justify-center order-1 md:order-2">
          <div className="scoreboard-card relative w-full max-w-sm bg-[#0f0f1a] border border-white/10 rounded-lg shadow-[0_30px_80px_-20px_rgba(0,0,0,0.6)]">
            ...
            <div className="grid grid-cols-3 divide-x divide-white/10 py-6">
              {[
                ...
                <div key={i} className="text-center px-2">
                  <p className="font-arena text-[#ff7a1a] text-2xl md:text-3xl font-black">
                    {s.value}
                  </p>
            ...
            <div className="mx-4 mb-5 bg-black/40 border border-white/10 rounded px-4 py-3 overflow-hidden">
              <p
                key={tick}
                className="font-arena text-[#ff9d4d] text-xs md:text-sm font-semibold tracking-wide"
              >
                ▸ {tickerLines[tick]}
              </p>
            </div>
