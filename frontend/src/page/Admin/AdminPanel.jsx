...
const NAV = [
  ...
  { key: "completed-matches",  label: "Completed Matches",  icon: "✅", roles: ["super-admin", "admin"] },
];

const Sidebar = ({ page, setPage, admin, onLogout, badges }) => (
  <aside style={{ width: 210, minWidth: 210, background: "#0f172a", display: "flex", flexDirection: "column", height: "100vh", position: "sticky", top: 0 }}>
    ...
    <div style={{ padding: 12, borderTop: "1px solid #1e293b" }}>
      <button onClick={onLogout} style={{ width: "100%", padding: "7px", background: "#1e293b", border: "none", borderRadius: 8, color: "#94a3b8", fontSize: 12, cursor: "pointer" }}>Sign out</button>
    </div>
  </aside>
);
...
const Topbar = ({ title, sub, onRefresh }) => (
  <div style={{ padding: "14px 24px", borderBottom: "1px solid #e5e7eb", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#fff" }}>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      ...
      <button onClick={onRefresh} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", background: "#3b82f6", color: "white", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>🔄 Refresh</button>
    </div>
  </div>
);
...
const ReqRow = ({ r, onApprove, onReject, actionLabel = "Approve" }) => (
  <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: "1px solid #f3f4f6" }}>
    ...
    <div style={{ flex: 1 }}>
      ...
      {r.trxId && <div style={{ fontSize: 10, color: "#3b82f6", marginTop: 1 }}>TRX: {r.trxId}</div>}
    </div>
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 14, padding: 16 }}>
          ...
          {recentDeposits.length === 0 ? <p style={{ fontSize: 12, color: "#9ca3af" }}>No data</p> : recentDeposits.map((h) => <HistRow key={h._id} h={h} />)}
        </div>
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 14, padding: 16 }}>
  ...
  return (
    <div style={{ padding: 24, maxWidth: 540 }}>
      <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 14, padding: 20 }}>
        <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 20 }}>🎮 New Match Create</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div><div style={{ fontSize: 11, color: "#6b7280", marginBottom: 4, fontWeight: 600 }}>Category</div>
            <select style={inp} value={form.category} onChange={(e) => setForm((p) => ({ ...p, category: e.target.value }))}>
              {[
                { key: "br_solo",   label: "Battle Royale Solo (48 players)" },
                { key: "br_duo",    label: "Battle Royale Duo (48 players)" },
                { key: "br_squad",  label: "Battle Royale Squad (48 players)" },
                { key: "cs_solo",   label: "Clash Squad Solo" },
                { key: "cs_duo",    label: "Clash Squad Duo" },
                { key: "cs_squad",  label: "Clash Squad (4vs4)" },
                { key: "cs_6vs6",   label: "Clash Squad 6vs6" },
                { key: "lw_solo",   label: "Lone Wolf Solo (1vs1)" },
                { key: "lw_duo",    label: "Lone Wolf Duo (2vs2)" },
          ...
          <div style={{ background: "#fefce8", border: "1px solid #fde68a", borderRadius: 10, padding: 14 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div><div style={{ fontSize: 10, color: "#92400e", marginBottom: 3 }}>🥇 1st Prize (৳)</div><input placeholder="60" {...fp("first")} style={{ ...inp, background: "#fff" }} /></div>
              <div><div style={{ fontSize: 10, color: "#92400e", marginBottom: 3 }}>🥈 2nd Prize (৳)</div><input placeholder="40" {...fp("second")} style={{ ...inp, background: "#fff" }} /></div>
              <div><div style={{ fontSize: 10, color: "#92400e", marginBottom: 3 }}>🥉 3rd Prize (৳)</div><input placeholder="20" {...fp("third")} style={{ ...inp, background: "#fff" }} /></div>
              <div><div style={{ fontSize: 10, color: "#92400e", marginBottom: 3 }}>4️⃣ 4th Prize (৳)</div><input placeholder="10" {...fp("fourth")} style={{ ...inp, background: "#fff" }} /></div>
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div><div style={{ fontSize: 11, color: "#6b7280", marginBottom: 4, fontWeight: 600 }}>Total Players</div><input placeholder="48" {...f("totalPlayers")} /></div>
            <div><div style={{ fontSize: 11, color: "#6b7280", marginBottom: 4, fontWeight: 600 }}>Per Kill (৳)</div><input placeholder="5" {...f("perKill")} /></div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          ...
          {msg && <div style={{ fontSize: 12, padding: "8px 10px", borderRadius: 6, background: msg.startsWith("✅") ? "#d1fae5" : "#fee2e2", color: msg.startsWith("✅") ? "#065f46" : "#dc2626" }}>{msg}</div>}
          <button onClick={submit} style={{ padding: "12px", background: "#111827", color: "#fff", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>🎮 Create Match</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ padding: 24 }}>
      <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 14, padding: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          ...
          <input placeholder="Search user..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ padding: "6px 10px", border: "1px solid #e5e7eb", borderRadius: 8, fontSize: 12, outline: "none" }} />
        </div>
        {filtered.length === 0 ? <p style={{ fontSize: 13, color: "#9ca3af", textAlign: "center", padding: 24 }}>No records</p> : filtered.map((h) => <HistRow key={h._id} h={h} />)}
      </div>
    </div>
  );
};
          ...
          <tbody>
            {filtered.map((u) => (
              <tr key={u._id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                ...
                <td style={{ padding: "9px 10px" }}><button onClick={() => toggleBan(u._id, u.isBlocked)} style={{ fontSize: 11, padding: "3px 8px", background: u.isBlocked ? "#d1fae5" : "#fee2e2", color: u.isBlocked ? "#065f46" : "#991b1b", border: "none", borderRadius: 6, cursor: "pointer" }}>{u.isBlocked ? "Unban" : "Ban"}</button></td>
              </tr>
            ))}
          </tbody>
        </table>
...
const MODE_CONFIG = {
  br_solo:    { logic: "solo_kill",  teamSize: 1, label: "BR Solo"     },
  br_duo:     { logic: "team_kill",  teamSize: 2, label: "BR Duo"      },
  br_squad:   { logic: "team_only",  teamSize: 4, label: "BR Squad"    },
  cs_solo:    { logic: "solo_pos",   teamSize: 1, label: "CS Solo"     },
  cs_duo:     { logic: "team_only",  teamSize: 2, label: "CS Duo"      },
  cs_squad:   { logic: "team_only",  teamSize: 4, label: "CS Squad"    },
  cs_6vs6:    { logic: "team_only",  teamSize: 6, label: "CS 6vs6"     },
  lw_solo:    { logic: "solo_pos",   teamSize: 1, label: "LW Solo"     },
  lw_duo:     { logic: "team_only",  teamSize: 2, label: "LW Duo"      },
  // backward compat
  br_match:   { logic: "solo_kill",  teamSize: 1, label: "BR Match"    },
  br_survival:{ logic: "solo_kill",  teamSize: 1, label: "BR Survival" },
  clash_squad:{ logic: "team_only",  teamSize: 4, label: "Clash Squad" },
...
const getMatchMode = (match) => {
  ...
  return {
    ...
    label:    cfg.label,
    // helper booleans
    isSoloKill: cfg.logic === "solo_kill",
    isTeamKill: cfg.logic === "team_kill",
    isTeamOnly: cfg.logic === "team_only",
    isSoloPos:  cfg.logic === "solo_pos",
    isTeam:     cfg.logic === "team_only" || cfg.logic === "team_kill",
  };
};
...
const MatchResults = () => {
  ...
  const isTeamMatch = mode?.isTeam || false;
  const isTeamKill  = mode?.isTeamKill || false;
  const isSoloPos   = mode?.isSoloPos  || false;
...
// ════════════════════════════════════════════════════════════════════════════
// COMPLETED MATCHES — Last 1 month, auto delete
// ════════════════════════════════════════════════════════════════════════════
const CATEGORY_LABEL = {
  ...
  lone_wolf: "Lone Wolf", training: "Training",
};
...
const CompletedMatches = () => {
  ...
  useEffect(() => {
    api("/admin/completed-matches").then((d) => {
  ...
  return (
    <div style={{ padding: 24 }}>
      <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 14, padding: 20 }}>
        <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 4 }}>✅ Completed Matches</div>
        <div style={{ fontSize: 12, color: "#9ca3af", marginBottom: 16 }}>১ মাস পর automatically delete হবে</div>

        {matches.length === 0 ? (
          <div style={{ textAlign: "center", padding: 40, color: "#9ca3af" }}>
            <div style={{ fontSize: 32, marginBottom: 8 }}>🎮</div>
            কোনো completed match নেই
          </div>
        ) : matches.map((m) => {
          const isOpen  = expanded === m._id;
          const catLabel = CATEGORY_LABEL[m.category] || m.category;
          const totalDist = (m.results || []).reduce((s, r) => s + (r.prize || 0), 0);
          const deleteDate = m.deleteAt ? new Date(m.deleteAt).toLocaleDateString("en-BD") : "—";

          return (
            <div key={m._id} style={{ border: "1px solid #e5e7eb", borderRadius: 12, marginBottom: 10, overflow: "hidden" }}>
              {/* Card Header */}
              <div
                onClick={() => setExpanded(isOpen ? null : m._id)}
                style={{ padding: "12px 16px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#f9fafb" }}
              >
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                    <span style={{ fontSize: 11, fontWeight: 700, background: "#dbeafe", color: "#1e40af", padding: "1px 8px", borderRadius: 20 }}>{catLabel}</span>
                    <span style={{ fontSize: 11, color: "#9ca3af" }}>Delete: {deleteDate}</span>
                  </div>
                  <div style={{ fontWeight: 700, fontSize: 14, color: "#111827" }}>{m.title}</div>
                  <div style={{ fontSize: 12, color: "#6b7280", marginTop: 2 }}>
                    {m.map || ""} • {m.joinedPlayers || 0} players • ৳{totalDist} distributed
                    {m.completedAt && ` • ${new Date(m.completedAt).toLocaleDateString("en-BD")}`}
                  </div>
                </div>
              ...
              {isOpen && (
                <div style={{ padding: "0 16px 16px" }}>
                  {/* Summary */}
                  <div style={{ display: "flex", gap: 12, flexWrap: "wrap", margin: "12px 0", padding: 12, background: "#f0fdf4", borderRadius: 8 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: "#15803d" }}>
                      💰 Total distributed: ৳{totalDist}
                    </span>
                    {m.winnerTeam && (
                      <span style={{ fontSize: 12, fontWeight: 700, color: "#7c3aed" }}>
                        🏆 Winner: Team {m.winnerTeam}
                      </span>
                    )}
                    <span style={{ fontSize: 12, color: "#6b7280" }}>
                      Per Kill: ৳{m.perKill || 0}
                    </span>
                  </div>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                    <thead>
                      <tr style={{ background: "#f9fafb" }}>
                        {["#", "Name", "Team", "Pos", "Kills", "Pos Prize", "Kill Prize", "Total"].map((h) => (
                          <th key={h} style={{ padding: "6px 8px", textAlign: "left", color: "#6b7280", fontWeight: 600, borderBottom: "1px solid #e5e7eb" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {(m.results || [])
                        ...
                        .map((r, i) => (
                          <tr key={i} style={{ borderBottom: "1px solid #f3f4f6" }}>
                            ...
                            <td style={{ padding: "6px 8px", fontWeight: 600, color: "#111827" }}>{r.inGameName || "—"}</td>
                            <td style={{ padding: "6px 8px", color: "#7c3aed" }}>{r.team || "—"}</td>
                            <td style={{ padding: "6px 8px" }}>{r.position || "—"}</td>
                            <td style={{ padding: "6px 8px" }}>{r.kills || 0}</td>
                            <td style={{ padding: "6px 8px", color: "#059669" }}>৳{r.posPrize || 0}</td>
                            <td style={{ padding: "6px 8px", color: "#dc2626" }}>৳{r.killPrize || 0}</td>
                            <td style={{ padding: "6px 8px", fontWeight: 800, color: r.prize > 0 ? "#059669" : "#9ca3af" }}>
                              ৳{r.prize || 0}
                            </td>
                          </tr>
                        ))}
...
const AdminPanel = () => {
  ...
  const titles = {
    ...
    "completed-matches": ["Completed Matches", "Last 1 month — auto delete হবে"],
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#f9fafb", fontFamily: "'Inter', sans-serif" }}>
      ...
      <main style={{ flex: 1, overflowY: "auto" }}>
        ...
        {page === "completed-matches" && <CompletedMatches />}
      </main>
    </div>
  );
};
