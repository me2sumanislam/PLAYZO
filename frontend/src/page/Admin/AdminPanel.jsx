...
const MatchResults = () => {
  ...
  const handleResultChange = (index, field, value) => { const updated = [...results]; updated[index][field] = value; setResults(updated); };
  ...
  const isAlreadyAdded = (userId, currentIndex) => results.some((r, i) => i !== currentIndex && r.userId === userId);
  ...
  const submitResult = async () => {
    ...
    setLoading(true);
    try {
      const response = await api(`/admin/matches/${selectedMatch._id}/result`, { method: "PUT", body: JSON.stringify({ results: results.map((r) => ({ userId: r.userId, inGameName: r.inGameName, position: Number(r.position) || 0, kills: Number(r.kills) || 0 })) }) });
      if (response.success) { alert("✅ Result submit হয়েছে!"); setResults([]); setSelectedMatch(null); setPlayers([]); loadMatches(); }
