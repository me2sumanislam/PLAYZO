...
const MatchList = ({ matches, onBack, onJoinSuccess, title }) => {
  return (
    <div className="bg-gray-100 min-h-screen max-w-[450px] mx-auto pb-24">
      ...
      <div className="p-3 space-y-3">
        {matches.length === 0 ? (
          <p className="text-center text-gray-400">No Match Available</p>
        ) : (
          matches.map((match) => (
            <MatchCard
              ...
              onJoinSuccess={(newBalance) =>
                onJoinSuccess && onJoinSuccess(match._id || match.id, newBalance)
              }
            />
          ))
        )}
