...
const API = ((import.meta.env.VITE_API_URL || 'https://playzo-vn8e.onrender.com') + '/api').replace(/\/api\/api/, '/api');
  ...
  const fetchData = async () => {
    ...
    try {
      const [balRes, histRes] = await Promise.all([
        axios.get(`${API}/users/balance`, { headers }),
        axios.get(`${API}/withdraw/my`, { headers }),
      ]);
